import { getProductWithLargestDiscount } from "./filter.js";
...
export async function getProductsByBigDiscount(params) {
  const datas = await fetch("data.json");
  const result = await datas.json();
  const regex = new RegExp(params, "i"); // 'i' untuk case-insensitive
  ...
  const productsWithLargestDiscount = getProductWithLargestDiscount(products);
  console.log(productsWithLargestDiscount);
